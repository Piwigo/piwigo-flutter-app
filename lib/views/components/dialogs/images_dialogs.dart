...
class EditImagesPage extends StatefulWidget {
  const EditImagesPage({Key key, this.catId, this.images}) : super(key: key);
...
class _EditImagesPageState extends State<EditImagesPage> {
  ...
  PageController _getPageController() {
    if(isPortrait(context)) {
      return PageController(initialPage: _page, viewportFraction: 7/8);
    }
    return PageController(initialPage: _page, viewportFraction: 1/2);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(appStrings(context).imageOptions_edit),
        centerTitle: true,
      ),
                    ...
                    InkWell(
                      onTap: () {
                        Navigator.of(context).push(
                            MaterialPageRoute(builder: (_) => SelectTagsPage(selectedTags: _currentTags, onConfirm: (tags) {
                              setState(() {
                                tags.forEach((tag) {
                                  if (!_tags.contains(tag)) _tags.insert(
                                      tags.indexOf(tag), tag);
                                });
                                _currentTags.clear();
